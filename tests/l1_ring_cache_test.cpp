...
TEST(L1RingCacheTest, UnknownKeyMisses) {
    L1RingCache cache;
    l1_init(&cache);
    CacheEntry out{};
...
TEST(L1RingCacheTest, PutThenGetHits) {
    L1RingCache cache;
    l1_init(&cache);
...
TEST(L1RingCacheTest, CapacityOverflowEvictsOldest) {
    L1RingCache cache;
    l1_init(&cache);
...
TEST(L1RingCacheTest, StaleSlotMissAfterEviction) {
    L1RingCache cache;
    l1_init(&cache);
...
TEST(L1RingCacheTest, ExpiredEntryMisses) {
    L1RingCache cache;
    l1_init(&cache);
