...
static constexpr const char* kMalformedInputBody =
    R"({"id": 1, "name": "incomplete")"; // Missing closing brace
...
int main() {
    ...
    // 1. request_headers
    {
        ...
        p->set_key(":path");
        p->set_raw_value("/api/data?fields=id,name");
        stream->Write(req);
        envoy::service::ext_proc::v3::ProcessingResponse resp{};
        stream->Read(&resp);
    }

    // 2. response_headers: set content-type to application/json
    {
        envoy::service::ext_proc::v3::ProcessingRequest req{};
    ...
    {
        envoy::service::ext_proc::v3::ProcessingRequest req{};
        req.mutable_response_body()->set_body(kMalformedInputBody);
        ...
        // Verify body mutation is NOT present (fail-open preserves original body)
