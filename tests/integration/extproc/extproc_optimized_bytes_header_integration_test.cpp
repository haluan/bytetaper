...
// Filtered path: ?fields=id removes "name" from {"id":1,"name":"Alice"} (23 bytes).
// Filtered output: {"id":1} = 8 bytes.
// r2 (ResponseHeaders): x-bytetaper-optimized-bytes must be "0" (placeholder)
// r3 (ResponseBody):    x-bytetaper-optimized-bytes must be "8" (filtered size, not 23)
...
int main() {
    ...
    // request_headers: ?fields=id → has_query_selection=true → filtered path
    ...
    // Response 2: response_headers — x-bytetaper-optimized-bytes must be "0" (placeholder)
    ...
    }
    {
        ...
        for (const auto& mutation :
            ...
            if (mutation.header().key() == "x-bytetaper-optimized-bytes" &&
    ...
    // Response 3: response_body — x-bytetaper-optimized-bytes must be "8" (filtered size)
