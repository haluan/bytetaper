...
// r3 (ResponseBody):    x-bytetaper-transform-applied must be "true"  (filtering fired)
static constexpr const char* kInputBody = "{\"id\":1,\"name\":\"Alice\"}";
static constexpr const char* kExpectedR2Value = "false";
static constexpr const char* kExpectedR3Value = "true";
...
int main() {
    ...
    // request_headers: ?fields=id → has_query_selection=true → filtering will fire
    ...
    // response_body: end_of_stream=true triggers the filter
    ...
    // Response 2: response_headers — x-bytetaper-transform-applied must be "false" (placeholder)
    ...
    }
    {
        ...
        for (const auto& mutation :
            ...
            if (mutation.header().key() == "x-bytetaper-transform-applied" &&
    ...
    // Response 3: response_body — x-bytetaper-transform-applied must be "true" (filter fired)
